...
1. **短视频优先。** 标准模式 45-55s，深度解析 45s-3min，电影解读 3-5min。前 3 秒必须有钩子，单画面元素停留不超过 3 秒（黄金 3 秒法则见 `shared/shared-rules` §5）。
2. **双轨铁律。** 确定轨由脚本/门禁执行，LLM 不碰（**含配置文件，见上文「确定轨文件只读」**）；创意轨由 LLM 发挥。门禁受阻只能改创意轨，不得动确定轨配置或引擎代码。
