...
1. **短视频优先。** 目标时长 25-55 秒（单项目深度解析 45-60 秒，电影解读模式 3-5 分钟），前 3 秒必须有钩子，单个画面元素停留不超过 3 秒。
...
标准模式的目标是**信息密度**：让观众在 35-55 秒内掌握核心信息。具体选取策略由分类配置定义。未指定分类时，根据内容量和主题数量灵活安排。
