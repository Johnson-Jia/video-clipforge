...
> **数据来源**：17 天三平台播放数据分析。各平台核心指标和用户行为差异显著。

| 平台 | 核心增长指标 | 用户行为 | 文案策略 |
|------|------------|---------|---------|
| **抖音** | 5s完播率 → 分享量 | 算法推流驱动，分享=增长 | hook 数据锚定 + 引导分享（"转发给你身边写代码的朋友"） |
| **视频号** | 完播率 → 社交分享 | 社交传播驱动，朋友圈/群聊 | 专业深度 + 转发引导语（"值得团队一起看看"） |
| **小红书** | 收藏率 → 搜索流量 | 收藏=干货认可，搜索=长尾 | SEO 关键词标题 + 干货感结构 + 实用标签 |

文案输出到 `douyin.md`，用 `## 抖音` / `## 视频号` / `## 小红书` 分段。

---

### 7.6 抖音文案生成

根据视频内容，生成 **3 套不同风格** 的抖音发布文案。

**文案必须保存到 `douyin.md`**，包含三平台文案（`## 抖音` / `## 视频号` / `## 小红书`）、各平台标签列表、评论区自评。
...
视频号用户通过社交关系链发现内容（朋友圈、群聊、看一看），文案策略：

1. **标题风格**：专业但不枯燥，有观点但不极端
   - ✅ "6 个 AI 代码大脑深度对比，差距不止十倍"
   - ✅ "本周 GitHub 最值得关注的 5 个项目"
   - ❌ "太强了""炸了"等纯情绪表达

2. **引导转发**：在文案末尾添加社交分享引导
   - "觉得有用就转发给团队看看"
   - "分享给你身边写代码的朋友"
   - "建议收藏，回头慢慢研究"

3. **标签策略**：职业导向标签（视频号用户更偏职场人群）
   - 核心：`#程序员` `#开源` `#AI`
   - 职业：`#技术管理` `#架构师` `#创业者`
   - 社交：`#值得收藏` `#团队分享`

4. **正文结构**：与抖音类似但更长，可多 1-2 句背景说明，整体节奏更沉稳

---

### 7.8 小红书文案生成

小红书用户通过搜索和推荐双通道发现内容，收藏率是核心指标：

1. **标题风格**：SEO 友好 + 干货感
   - ✅ "GitHub 必看！6 个 AI 编程工具深度对比（附使用教程）"
   - ✅ "程序员收藏｜本周最值得关注的 5 个开源项目"
   - ❌ "今天涨星最猛的项目"（太时效性，搜索长尾差）

2. **正文结构**：干货列表体，每段有实用价值
   - 开头：一句话说清这条内容的价值（"整理了本周最值得关注的 N 个项目"）
   - 每个项目：名称 + 一句话定位 + Star 数 + 亮点 + 适合人群
   - 结尾：收藏引导（"建议收藏，用到的时候方便找"）

3. **标签策略**：搜索导向标签（小红书标签直接影响搜索曝光）
   - 必选：`#GitHub` `#开源项目推荐` `#程序员必看`
   - 搜索热词：`#AI工具推荐` `#编程效率` `#开发者工具`
   - 长尾：`#开源项目合集` `#GitHub每日热门` `#代码工具`
   - 标签数：8-10 个（多于抖音和视频号）

4. **封面文字**：小红书封面文字直接影响点击率
   - 必须包含数字（"6个项目""TOP5"）
   - 必须包含价值关键词（"必看""收藏""对比""推荐"）
   - 字号要大（手机缩略图下仍可辨认）
