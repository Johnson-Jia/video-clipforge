...
|------|--------|---------|
| 标准模式 | 6-8 个 | 45-55 秒 |
| 单项目深度解析 | 8-15 个 | 45 秒-3 分钟 |
| 电影解读模式 | 不限 | 3-5 分钟 |

## 标准模式 — 单项目全屏结构（8 层信息）

> **（爆款案例分析）** 采用一屏一项目布局，每项目包含 8 层信息。对比一屏多项目（5 层信息），播放量差 3 倍。
...
> **（58 条视频分析验证）**
> - 5s 完播率 ≥44% → 平均 36K 播放（11.6x 基线）
> - 5s 完播率 38-44% → 平均 12K 播放（4.0x 基线）
> - 5s 完播率 <38% → 平均 3K 播放（基线）
...
> **执行前读 `shared/cinematic-shotlist`**。每个 scene 必填 3 字段（创意轨，据情绪+内容选值）：

| 字段 | 取值 | 选取依据 |
|------|------|---------|
| `shot_size` 景别 | 大远景/远景/全景/中景/近景/特写/大特写 | 情绪（钩子→特写、环境→远景）+ 内容（细节→特写、数据→中景）|
| `camera_move` 运镜 | 固定/推/拉/摇/俯仰/移/跟/手持/环绕/变焦/第一视角/荷兰角 | 节拍（grab→推/手持、settle→固定）+ 张力 |
| `transition` 转场 | 硬切/叠化/淡入/淡出/黑场 | 节奏（快切→硬切、抒情→叠化）+ 段落分隔 |

字段缺失/非法 → 软校验警告 + 默认兜底（中景/固定/硬切），不阻塞但拉低电影感。

**情绪→字段速查**见 `shared/cinematic-shotlist.md`。LLM 选值（创意自由），脚本映射渲染（流程固化，s6_assemble build_gsap 自动注入运镜相机动画 + 转场过渡）。
...
### 单项目深度解析 — 8 场景模板（深度解析支持 8-15 场景，下为 8 场景骨架示例）

> **结构原则（核心认知）**：深度解析要让陌生观众**先共鸣再讲深度**——技术细节后置服务场景，不前置炫技。

- **前半段**（hook + what + usecases）= 痛点 + 应用场景 + 使用人群，让陌生观众代入"这跟我什么关系"
- **后半段**（how + tech + capabilities）= 架构深度（架构图/原理/协议），讲"怎么做到的"
- 前半段只讲"解决什么痛点、谁能用"；架构图/技术原理放后半段

> **⛔ 项目名必报（必查·LLM 自审，暂无 gate——引擎化待办：复用 content_ready 项目集解析对 what/cta 段校验）**：S2（what 场景）旁白必须报项目名（"它叫 XXX"）+ 画面项目名大字；S8 CTA 强化；封面 badge 含项目名；全程 ≥3 次接触（画面/旁白/CTA/封面）。**禁全程用"它/这个工具"代词**——观众看完不知是什么项目。hook（S1）仍不报名（前 5 秒纯钩子）。
  ...
  },
  {
    ...
    "visual_phases": [
      ...
      },
      {
        "focus": "中国市场出货量",
        "visual_type": "data",
        "narration_anchor": { "start_sentence": 2, "end_sentence": 3 },
        "key_data": ["腕戴7390万台", "智能手表5061万台", "智能手环2329万台"]
      },
      {
        "focus": "渗透率与确定性",
        "visual_type": "highlight",
        "narration_anchor": { "start_sentence": 4, "end_sentence": 5 },
...
| `visual_intent` | object/null | **导演视觉意图**。每场景 × 三层（bg/fx/content）的视觉指导，供 Stage 6 §6.10 组件匹配使用。短场景（≤4s）可传 null |
...
7. **narration_anchor 必填**（有 visual_phases 的场景）。每个 phase 必须标注 `narration_anchor`，指定该 phase 对应旁白中哪些句子。这是 phase 时间自动校准的唯一数据源

### narration_anchor 定义

> narration_anchor 精确标注方法（按句拆分、0-index、±50ms 精度）详见 `shared/narration-anchoring.md`。核心规则：每个 phase 必须标注 `narration_anchor`，值为该 phase 旁白中按句号拆分后的**第 N 个句子**的索引。

### visual_intent 编写规则

为每个场景的三层分别描述导演的视觉意图，供 Stage 6 §6.10 组件匹配使用。
...
- 总字数控制在 **200-350 字**（标准模式）或 **300-950 字**（深度解析模式，≤3min）
...
- YunjianNeural +25%：每秒约 5.3 字（中文含标点停顿实测；旧值 8.5-9 偏高约 40%，深度解析按 5.3 算字数上限）
- 长文本（>100 字/段）：TTS 实际略快于公式，预估时可保守按上限计算

| 模式 | 目标时长   | 建议字数 |
|------|--------|---------|
| 标准模式（5-6 个项目） | 45-55s | 250-380 字 |
| 深度解析模式 | 45s-3min | 300-950 字 |
