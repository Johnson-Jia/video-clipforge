...
### 1.2 翻译腔套路句规避（质感引导，SOFT）

> **"不是X，而是Y" 是英语 "not X but Y" 直译的翻译腔**，知识区/科技区过度滥用已成套路，观众一听就出戏（"又来了"）。本质是**内容质感问题**（非平台合规，不违规），但影响观众体验与完播。

**为什么规避：**
- 翻译腔生硬——中文口语几乎不这么说话，书面感破坏短视频叙事节奏
- 否定前置（"不是X"）是冗余铺垫，占用信息密度，不如直接说"是什么"
- 潜台词"你以为X，其实Y"带说教/纠正感，削弱"朋友分享"的亲和力
- 套路化导致审美疲劳，是 6 月知识区同质化的隐性播放杀手

**替换原则：直接肯定句优先，不用"不是…而是…"转折。**

| ✗ 翻译腔套路 | ✓ 口语直接 |
|-------------|-----------|
| 它不是要替代 Windows，而是补上效率功能 | 它补上的，是 Windows 自己一直没做的效率功能 |
| 它不是从零造轮子，而是整合社区 | 它直接收编了社区里已经成熟的好工具 |
| 这不是工具，而是生产力革命 | 这套工具，直接把日常效率拉满 |

**适用范围：** 旁白（Stage 3）、发布文案（Stage 7）。偶尔用于强对比并非绝对错，但单个视频避免超过 1 处，且必须有明确的对比价值；默认优先用直接肯定句。
...
**HARD，gate: no_app_name** — 禁止出现具体商业 app/软件名（剪映是字节系产品，提它做"替代"触发抖音封禁），用功能泛化。
...
- 钩子句参考案例（不是模板，从内容中寻找最有力的一击）：
  ...
  - **数字+利益（双钩子，推荐）**："三天涨了两万星，它能帮你 XXX"
  - **大厂+悬念（双钩子，推荐）**："大厂刚开源的，赌你没用过"
  - ⛔ 禁裸报数字（"X 千星的项目"无利益）—— 必须配观众能懂的好处
...
- Phase 间切换由 GSAP timeline 驱动（详见 `shared/render-safety.md` §1.2 和 `stages/stage6-production.md` §6.11）
