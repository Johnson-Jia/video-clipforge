...
BRAND_PRONUNCIATION_MAP = {
    ...
    # 开发工具
    'Copilot':    '副驾驶',
    'Yazi':       '鸭子',         # Yazi 终端文件管理器，TTS 读字母/拼音，映射"鸭子"
    'HuggingFace': '哈金Face',
    'Supabase':   '苏帕贝斯',
    'Vercel':     '维尔塞尔',
    'Netlify':    '奈特利菲',
...
_CUSTOM_PHRASES = {
    ...
    '长时间': [['cháng'], ['shí'], ['jiān']],
    # ── 长(cháng) 补充（2026-08-03 长答案事故）— pypinyin 对这些误推 zhang3，靠 _LONG_AS_CHANG_RE 替换"常"兜底 ──
    '长答案': [['cháng'], ['dá'], ['àn']],
    '长度': [['cháng'], ['dù']],
    '长久': [['cháng'], ['jiǔ']],
    '长城': [['cháng'], ['chéng']],
...
# 调(tiáo) 上下文 — 「调 DPI」(调+英文参数)、「全能调」(句尾调整义)，防 edge-tts 读 diào(调查)
...
# _POLYPHONE_MAP 的 长:{zhang3:掌} 会误改成「掌」→ TTS 读 zhǎng（错，应 cháng），
# 故这些 cháng 上下文必须在 _POLYPHONE_MAP 之前由本正则替换为「常」
_LONG_AS_CHANG_RE = re.compile(r'长(视频|篇|文|尾|期|线|河|途|任务|答案|度|久|城|假|效)')
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        tests = [
            ...
            # 品牌名 + 多音字组合
            ('Qwen跑了一行命令', '千问跑了一航命令'),
            # Yazi 项目名发音（大小写不敏感，词边界）
            ('Yazi 终端文件管理器', '鸭子 终端文件管理器'),
            ('yazi 很快', '鸭子 很快'),
            ('用了Yazi之后', '用了鸭子之后'),
            # 长视频定向替换
            ('长视频', '常视频'),
            ...
            ('企业销售周期长', '企业销售周期常'),
            ('周期长是客观现实', '周期常是客观现实'),
            # 长(cháng) 上下文 — 防 pypinyin 误推 zhang3 → _POLYPHONE_MAP 误改「掌」
            ('长答案', '常答案'),
            ('别把结论埋在长答案里', '别把结论埋在常答案里'),
            ('长度限制', '常度限制'),
            ('长久之计', '常久之计'),
            ('长城不是一天建成的', '常城不是一天建成的'),
            # 长(zhǎng) 上下文 — pypinyin 推 zhang3 正确，走 _POLYPHONE_MAP → 掌（不误伤）
            ('长大想做科学家', '掌大想做科学家'),
            # 阿拉伯数字+行
            ('300行代码', '300航代码'),
            ('100多行代码', '100多航代码'),
            ...
            ('290K', '二十九万'),
            ('28.8K', '二万八千八百'),
            ('Stars从1500暴涨到28800', 'Stars从1500暴掌到28800'),  # 涨 zhǎng→掌（edge-tts 读 zhang4 错，强制掌）；本测试主旨是验证数字不被 K 转换
            # 发音易错词
            ('这能干嘛', '这能干什么'),
            ('你能干嘛呢', '你能干什么呢'),
