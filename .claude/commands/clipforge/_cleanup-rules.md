...
>
> **严禁手动 `rm -f` 批量删除文件。** 2026-05-22 和 2026-05-27 两次事故均因手动 `rm -f` 删除了保留清单中的文件（cover.html、index.html、design.md、narration_segments.json 等），导致微调视频需要重跑整个阶段。
...
## 清理前检查点（2026-05-27 加固）

> **执行任何删除操作前，必须先完成此检查点。** 跳过此检查点直接删除 = 事故。
