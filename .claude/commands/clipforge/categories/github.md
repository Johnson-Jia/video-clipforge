...
**⛔ 项目重复硬约束（HARD，gate `project_no_consecutive_repeat` 拦截）：**

| 场景 | 规则 |
|------|------|
| **同频道跨日（近2天）** | 昨日/前日**同频道**已入选的项目，本期**禁选**——换角度讲同一项目不算差异化，stage1 gate 硬拦。唯一例外：该项目今日 `stars_today` 涨进当日 raw **前3**（真爆款），可一句带过增量不展开 |
| **跨频道** | **不校验**——daily（综合）与 ai-wind（AI专项）是不同频道，各自独立检测（ai-wind 内部连续 / daily 内部连续才拦），互不拦截。同日两榜可选同一项目 |
| **weekly 周榜** | 周榜是"一周热门汇总"回顾属性，与近2天 daily 重叠天然合理，**豁免**此门禁 |

**⛔ AI 项目占比硬约束（HARD，gate `ai_project_cap` 拦截）：**

> 有了 **ai-wind（AI 风向标）** 专项专辑承接 AI 项目深度展开后，**daily 综合榜应回归多元**——AI 项目数 **0-2 个**（**上限是 2，不是必须凑满 2 个**），把名额让给工具/基建/安全/硬件/前端等非 AI 方向，避免两专辑内容高度重叠。

| 适用 | 规则 |
|------|------|
| **github-trending 综合4榜** | AI 项目 **0-2 个**（上限 2，可 0/1/2，非必须 2）。stage1 gate 硬拦超出。AI 判定 = ai_trending.py 同款（topics 命中 AI_TOPICS 或 description 命中 AI 关键词） |
| **ai-wind AI 专项** | 不限（本就是 AI 专辑，gate 跳过） |
| **weekly 周榜** | 豁免（回顾属性） |

**⛔ 跨频道 AI 去重（SOFT）：** daily 选 AI 项目时，**避开昨日 ai-wind 已选的 AI 项目**（inject 会注入清单）——观众同一天/连续两天刷到同一 AI 项目会觉重复。虽 gate 仅同频道硬拦，但跨频道（daily ↔ ai-wind）观众视角重复也要主动避开。非 AI 项目不受此约束。

**选材策略**：daily 优先选非 AI 多元方向（PDF/文档工具、JS 运行时、测试框架、CSS 框架、日志库、项目管理等），AI 项目按内容自然决定 0-2 个（不硬凑）。若 trending 非 AI 热门项目不足，**先按下方「安全工具入池规则」审视 OSINT/安全工具是否可中性化入池**；仍不足才少选不突破 AI 上限（差异化优先于凑数）。

### security_tools — 安全工具入池规则

> **⛔ 非 AI 池枯竭时，先审视 OSINT/安全工具是否可中性化入池，勿笼统判"灰色剔除"跳过 daily。**

Red Flags 零容忍的是 **VPN 翻墙 / IPTV 盗版直播源 / 破解盗链** 等违法盗版工具。**合法开源安全工具（OSINT / 攻击面测绘 / 隐私自查 / 威胁情报）可入池**——它们是 GitHub 长期存在的正经安全研究工具，属「安全/隐私」选题（每周 ≤1，低频高质）。

| 可入（中性化讲自查/防御） | 剔除（真灰色/违法） |
|---|---|
| 攻击面测绘（spiderfoot）| VPN 翻墙（MasterDnsVPN）|
| 邮箱注册痕迹自查（holehe）| IPTV 盗版直播源（iptv-org）|
| 威胁情报/漏洞扫描（OpenVAS 类）| 破解/盗链/游戏外挂 |
| 密码强度自查 / 隐私审计 | 换脸/脱衣/伪造证件 AI |

**⛔ 中性化铁律（HARD，平台合规红线）：**
- ✅ 只讲「**自查自己的信息 / 防御 / 给自己做安全体检 / 翻出自己的注册记录**」
- ❌ 禁讲「侦察 / 社工 / 攻击他人 / 查别人 / 渗透靶标」
- 画面禁用攻击性视觉（骷髅/红色警报/靶标/入侵动画），用防御视觉（盾牌/扫描雷达/报告/锁）

**受众分档参考**：spiderfoot（A·网页界面自查）/ holehe（B·命令行自查）/ 密码审计类（A·网页）。不活跃的老工具（如 holehe 700+ 天未更新）可按「经典老工具」角度入（参照 nvm 长青叙事），不因不活跃一刀切剔除。
...
当视频只聚焦一个项目时，基础数据（Star/Fork/Language）不足以支撑 45 秒-3 分钟的深度内容。需要额外调研以下维度：
...
| owner/repo | 排除原因 |
|------------|---------|
| ruvnet/RuView | WiFi 信号感知人体，虚假宣传/普通人无法实现 |
...
- ⛔ **领域色只用于视觉**（卡片色环/边框/标签/渐变标题文字），**旁白与画面文字禁说颜色词**（"冷蓝""暖橙""紫色""金色""绿色"等）。颜色是视觉概念，观众听/看颜色词无意义且突兀；旁白按**项目内容/领域名**分组引导（"AI Agent 这组""视频创作工具""安全隐私"），不用颜色词。
...
> **按模式分钩子策略（核心认知）**：钩子要对**陌生观众**讲痛点，不是炫技术反差。

| 模式 | 钩子策略 | 原因 |
|------|---------|------|
| 多项目盘点（trending/weekly） | 反直觉/冲突 + 数字锚定 | 观众多为目的性刷榜，反差+数字抓眼球 |
| **单项目深度解析** | **痛点场景**（让观众代入"我也有这困扰"） | 陌生观众不知项目，技术反差（"快/慢"）无参照系，只有痛点能让人停下 |

**单项目深度解析钩子**（痛点优先，禁纯技术反差）：
- ✗ "号称最快却复制不快"（观众不知项目，反差无意义）
- ✓ "终端里想看张图，还得切出去开软件？"（痛点共鸣，让人想知道解法）

> **数据来源（44 个项目三维分析验证）：** 反直觉/冲突类 5s 完播率最高（glm-vs-qwen 53.1%）；动作型词"杀入/冲上"平均 49K 播放；"炸"单次有效 141K 但不可连续使用；"最猛"已疲劳化（均值 8K / 29.6% 5s），从模板中移除。
...
> **⛔ 利益翻译铁律**：任何含数字/大厂名的钩子，**必须配一句观众能带入的利益翻译**，禁裸报数字。
...
> **⛔ hook 具象度反例（c5s 下钻验证）**：
> 6月高 c5s（0.40+）hook 均 ≤15字 + 具象数字/冲突词；7月低 c5s（0.32）hook 变长/抽象/堆叠/元铺垫。
> - ✗ "AI居然替你跑全流程"（contrarian 但抽象无数字，c5s 0.22）
> - ✗ "几个猛项目冲进GitHub"（signal_attention 模糊词，c5s 0.33）
> - ✗ "不用装软件造3D建筑。今天GitHub上的项目，方向挺杂"（38字+元铺垫「方向挺杂」，c5s 低）
> - ✓ "给AI的输入砍掉95%，答案一模一样"（contrarian+具象数字95%+反直觉收益，c5s 0.42）
> - ✓ "输入砍掉九成，AI答案没变"（15字+数字九成+反直觉，c5s 高）
> gate check_hook_pattern_verified 已 HARD 拦截（字数>20/堆叠连接词/元铺垫/无数字或冲突词）。
...
### ending_question — 结尾提问（禁站队对抗式）

> ⛔ **提问规则**：正文零提问（保持干货密度 + 节奏），**结尾仅 1 个二选一问题** + "关注我下期见"。
> - **用「想尝试 / 会用 / 会试 哪个」类中性互动**：「这几个你最想尝试哪个」「Pake 和 palmier-pro 你会试哪个」「这个你会用上吗」——让观众代入使用场景，自然引发评论
> - **禁站队/对抗式**：「你站谁」「X 和它你站谁」「你敢说真香吗」「我吹爆你敢说吗」——"站谁"营销感强、对抗意味重，观众反感（覆盖旧"站队评论率高 3-5×"建议）
> - 禁每个项目都问（啰嗦掉完播）；禁纯中性（"你觉得呢"评论率低）——要二选一但非对抗
> - 问题类型轮换（想尝试 / 会用 / 会试），禁连续两期同款句式（反同质化）
> - 措辞自然（遵守 shared-rules §1 禁诱导互动：不"快评论/求点赞"，是内容讨论口吻）

### word_count_range

标准模式：250-380 字。深度解析模式：300-950 字（≤3min）。
