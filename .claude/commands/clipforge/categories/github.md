...
> 有了 **ai-wind（AI 风向标）** 专项专辑承接 AI 项目深度展开后，**daily 综合榜应回归多元**——AI 项目数 **≤ 2 个**（5-6 项目里最多 2 个 AI），把名额让给工具/基建/安全/硬件/前端等非 AI 方向，避免两专辑内容高度重叠。

| 适用 | 规则 |
|------|------|
| **github-trending 综合4榜** | AI 项目 ≤ 2（stage1 gate 硬拦，超出换非 AI 项目补）。AI 判定 = ai_trending.py 同款（topics 命中 AI_TOPICS 或 description 命中 AI 关键词） |
| **ai-wind AI 专项** | 不限（本就是 AI 专辑，gate 跳过） |
| **weekly 周榜** | 豁免（回顾属性） |

**选材策略**：daily 优先选非 AI 多元方向（PDF/文档工具、JS 运行时、测试框架、CSS 框架、日志库、项目管理等），保留 ≤2 个最热门 AI 项目维持流量。若 trending 非 AI 热门项目不足，宁可少选也不突破 AI 上限（差异化优先于凑数）。
