---
name: "专业教程"
description: "横屏16:9企业级技术教程视频，13-16分钟深度解析（整集口径）"
id: "tutorial"
---
...
### word_count_range
4000-4800 字（13-16 分钟深度解析）。**整集口径**——ClipForge 教程按段级制作（每段一个项目目录，段 narration 为百字级），段字数以 `ENX-脚本.md` 对应段为准，本区间约束的是整集脚本总字数。实测 TTS（YunjianNeural +0%）约 300 字/分（每分钟约 280-320 字，~5 字/秒）。**字数预算 = 目标秒数 × 5**。

### special_rules
- **专业 + 口语**：不是短视频的"炸/霸榜/最猛"，是"这一步我们做的是…""关键在哪呢"。教程类要让人跟上，不是刺激。
- **统一结构**：钩子 30s → 这集讲什么 30s → 主体 10-13min → 案例证明 1.5min → CTA 30s（合计 13-16min）
