...
引擎层完整设计参见 [自进化架构设计](docs/Agent自进化架构设计.md)。
...
└── clipforge/
    ...
    ├── components/                # 视觉组件库（52 个三层组件）
    │   ├── bg/                    # 背景层（15 个：极光、星云、光束…）
    │   ├── fx/                    # 特效层（14 个：粒子、光带、代码雨、星座…）
    │   └── content/               # 内容层（23 个：数据卡、时间线、电影特效…）
    ├── scripts/                   # 工具脚本
    ├── engine/                    # 自进化引擎（门禁/归因/Trace/治理/可观测性/lint）
    ├── rules/                     # 约束规则库
    ├── skills/                    # 技能声明（四原子模型）
    └── patterns/                  # 经验模式（数据驱动）
```
...
| **内容层** (content) | `components/content/` | 23 | 承载信息呈现 — 数据卡、时间线、对比面板 | 英雄卡片、星级计数、数据可视化、结论框 |
