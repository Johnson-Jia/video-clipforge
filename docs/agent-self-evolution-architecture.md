...
| **门禁评估** | 仅 process + compliance | process + compliance + quality | process + compliance + quality + 人工复核（首次 N 条） |
...
**STRICT 模式的实现要求**：
- **人工复核**：新 Skill 上线的前 N 次执行（默认 N=5），即使流程/合规门禁全部通过，也需人工确认后才标记为 PASSED
- **模式入库确认**：正向闭环产出的经验模式在写入 Pattern Store 前需人工确认
- **归因审计**：每次归因产出 Delta Rule 时，记录完整审计日志（归因证据链、置信度推导过程）
...
每个 Skill 声明 `max_retries`（默认 2），流程/合规门禁驳回达到上限后：
- 停止执行
- 输出当前最佳结果 + 失败原因
- 轨迹完整记录，进入归因
...
```
  ...
  │ 质量门禁  │     进入负向闭环
  │ quality  │
  │ HUMAN    │──→ 记录 quality_score
  └──────────┘     外部回填后触发正向闭环
```

**正向闭环的触发条件**：满足 §6.3.3 中定义的三种触发方式之一时，视为"高分成功案例"，进入正向闭环。

#### 6.3.2 三层门禁模型

门禁分为三层，前两层属于**协议域**（确定性、可审计），第三层属于**生成域**（创意评价）：

| 层级 | 名称 | 检查方式 | 不通过动作 | 域归属 |
|------|------|---------|-----------|--------|
| **process** | 流程门禁 | 自动化脚本（文件存在性、脚本执行、格式校验） | 立即驳回 → 归因 | 协议域 |
| **compliance** | 合规门禁 | 关键词/正则匹配（敏感词、URL、语言规范） | 立即驳回 → 归因 | 协议域 |
| **quality** | 质量门禁 | 人类评价或外部数据回填（创意质量、视觉冲击力） | 记录但放行 | 生成域 |

**关键设计**：流程门禁和合规门禁共同构成协议域的校验层——它们用确定性方法检查产出物是否满足硬性要求。质量门禁是生成域的评价层——它不干预 Agent 的创作路径，只在产出完成后由人类或外部数据评价创意质量。

#### 6.3.3 正向闭环触发条件

正向闭环（§6.7）的触发条件有三种方式：

1. **外部数据触发**（首选）：视频发布后，播放数据回填到 Trace 的 `quality_score` 字段（`evaluator: PLAYBACK_DATA`），`quality_score ≥ 0.85` 时触发
2. **人工评价触发**：人工审看后回填 `quality_score`（`evaluator: HUMAN`），`quality_score ≥ 0.85` 时触发
3. **流程完整性触发**（降级方案）：`process_passed: true` AND `compliance_passed: true` 时，记录为"流程完整案例"，产出 SEED 模式（需更多案例验证）
...
trace:
  ...
  result:
    output: "..."                           # 脱敏后的输出
    gate_report:
      process_passed: true                  # 流程门禁通过
      compliance_passed: true               # 合规门禁通过
      quality_score: null                   # 质量评分（外部回填）
...
当一次执行满足正向闭环触发条件（§6.3.3）时，视为**高分成功案例**，触发采集：
...
pattern:
  ...
  evidence:
    ...
    avg_quality_score_without_pattern: 0.72   # 对比基线
...
**制衡规则**：
- 每条经验模式入库前，必须通过流程/合规门禁校验——如果模式本身包含违规内容，负向闭环有权否决
...
| PatternDeposit | 模式治理 | 模式库 | 流程/合规门禁校验通过 |
...
| 流程/合规门禁驳回率 | 驳回次数 / 总执行次数 | > 30% → 检查约束注入是否有效 |
...
| **校验引擎** | process + compliance + quality 三层 Gate 评估 | 结果 + Gate 定义 | ValidationReport |
...
| **Gate** | 结果校验标准，分三层：process（流程门禁，脚本检查）、compliance（合规门禁，正则匹配）、quality（质量门禁，人类评价）；前两层属协议域，第三层属生成域 |
| **流程门禁（process）** | 自动化脚本检查（文件存在性、脚本执行、格式校验），不通过=驳回，属于协议域 |
| **合规门禁（compliance）** | 关键词/正则匹配（敏感词、URL、语言规范），不通过=驳回，属于协议域 |
| **质量门禁（quality）** | 创意质量评价（evaluator: HUMAN 或 PLAYBACK_DATA），不通过=记录但放行，属于生成域 |
